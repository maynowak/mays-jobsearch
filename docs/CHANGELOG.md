...
- **Tests:** `tests/api/providers.test.mjs` (Router: Katalog-Aggregation/Dedup, Provider-Zuordnung, Provider-Fallback bei Quota, Durchreichen von `model_unavailable`, `limit_reached`, `missing_key`) und `tests/api/edenai-provider.test.mjs` (Key-Auswahl, Pricing-basierte Free-Eligibility, Fallback-Präferenz, Chat-Request, `response_format` nur bei strukturierten Modellen, 401/402/429/5xx-Mapping, Regression: Erfolgs-Response-Body wird nur einmal gelesen). Alle bestehenden Tests bleiben grün (70 gesamt).
- **Fixes (Live-Verifikation):** Erfolgs-Response wurde doppelt gelesen (`response.json()` zuerst für die Fehlermeldung, dann für den Body) — jeder erfolgreiche EdenAI-Call wurde fälschlich als `model_unavailable` geparst; Fehler-Body wird jetzt nur in den Fehlerzweigen gelesen. EdenAI-Provider-Timeout von 40 s auf 55 s erhöht (langsame Reasoning-Modelle bei Cold Calls). Standard-Modell `cloudflare/@cf/google/gemma-7b-it-lora` statt des Reasoning-Modells `google/gemma-4-26b-a4b-it` (dessen `reasoning_content` schluckt das gesamte Token-Budget und liefert `content: null`); Fallback-Ranking bevorzugt Nicht-Reasoning-Modelle.
- **Docs:** neue `docs/AI_PROVIDERS.md`; ARCHITECTURE, COMPONENT_GUIDE, DEPLOYMENT, README und CHANGELOG aktualisiert.

## Unreleased — 2026-08-20

Precise Model Fallback Feedback + Suchparameter-Erweiterung.

- **Precise fallback feedback:** bei `model_unavailable` wird präzise gemeldet, welches Modell nicht verfügbar war und welches als nächstes versucht wird; die bereits gefundenen Jobs bleiben erhalten. Nach endgültigem Fehlschlag kann ein anderes Modell gewählt werden, ohne die Jobs erneut zu laden (kein `/api/jobs`, kein Apify).
- **Manueller Modell-Retry (UX-Korrektur):** eine Modellwahl startet kein Matching automatisch; die Suche/ das Matching wird über den kontextabhängigen Button („Mit diesem Modell erneut bewerten") manuell gestartet. Suchparameter-Änderung invalidiert das Dataset (keine automatische Suche), Modellwechsel nicht.
